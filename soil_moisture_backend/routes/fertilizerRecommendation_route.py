...
bp = Blueprint('fertilizerRecommendation', __name__,template_folder='templates', static_folder='static')

# Load the model
FERTI_NAME_MODEL_PATH = os.path.join(os.path.dirname(__file__), '../models/rf_ferti_name.pkl')
rf_ferti_name = joblib.load(FERTI_NAME_MODEL_PATH)

FERTI_VALUE_MODEL_PATH = os.path.join(os.path.dirname(__file__), '../models/rf_ferti_value.pkl')
rf_ferti_value = joblib.load(FERTI_VALUE_MODEL_PATH)
...
def generate_ai_suggestions(pred_fertilizer_name):
    prompt = (
        f"For {pred_fertilizer_name} fertilizer, generate 3-4  sentences each on a new line, note text should be justified should not contain any special character"
    )
...
def index():
    if request.method == 'POST':
        ...
        return render_template('fertilizerRecommendation/index.html', prediction=True, fertilizer_name=pred_fertilizer_name,
                               fertilizer_qty=pred_fertilizer_qty, optimal_usage=pred_fertilizer_qty,pred_info=pred_info)
    return render_template('fertilizerRecommendation/index.html', prediction=False)
