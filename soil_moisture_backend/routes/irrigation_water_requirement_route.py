...
bp = Blueprint('irrigation_water_requirement', __name__,template_folder='templates', static_folder='static')

# Load the SVM model
MODEL_PATH = os.path.join(os.path.dirname(__file__), '../models/svm_poly_model.pkl')
svm_poly_model = joblib.load(MODEL_PATH)
...
@bp.route('/')
def index():
    return render_template('irrigation_water_requirement/index.html')


@bp.route('/fetch_weather', methods=['GET'])
def fetch_weather():
...
def predict():
    if request.method == 'POST':
        ...
        return render_template('irrigation_water_requirement/predict.html',
    ...
    return redirect(url_for('irrigation_water_requirement/index'))
